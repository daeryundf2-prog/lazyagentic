#!/usr/bin/env node
// intent-guard.mjs — PreToolUse command checks + Stop turn audit (rules-only repo, no auto-registration)
// FAIL_OPEN -> ask: 파싱 실패/전사 누락 시 차단하지 않고 approve(확인 질문으로 전환)
// Input: hook JSON via stdin { event, transcript_path, tool_name, tool_input, ... }. Output: decision JSON to stdout.
